...
@Component({
  ...
  templateUrl: './course-feedbacks.html',
  styleUrl: './course-feedbacks.css',
})
export class CourseFeedbacks implements OnInit {
  constructor(private route: ActivatedRoute) { }
  id: number = 0;
  ngOnInit(): void {
    this.id = Number(this.route.snapshot.paramMap.get('id'));
    this.getFeedBackByCourseId(this.id);
  }
  http = inject(HttpClient);

  feedbacks: Feedback[] = [];
  getFeedBackByCourseId(id: number) {
    debugger;
    this.http.get<Feedback[]>(`https://localhost:7212/GetFeedbacksByCourseId+${id}`)
      .subscribe({
        next: (data) => {
          this.feedbacks = data;
